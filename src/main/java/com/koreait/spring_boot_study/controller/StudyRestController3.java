...
public class StudyRestController3 {
    ...
    GET 요청을 제외한 모든 요청 메서드에는 body가 존재한다.
    ...
    */
    ...
    public String test2(@RequestBody StudyReqDto dto) {
        // 내가 정의한 클래스의 객체로 데이터를 받을 때
        // 주의)  필드명과 JSON key 이름이 동일해야한다.
        log.info("test2 컨츠롤러 수신");
        log.info("들어온 데이터: {}", dto);
        return "성공";
    }

    // 실습) test3 라는 이름으로 AddPostReqDto 타입의 데이터를 수신
    // PostMapping
    @PostMapping("/test3")
    public ResponseEntity test3(@RequestBody AddPostReqDto dto) {
        ...
        */
        ...
        403: 권한이 없음 FORBIDDEN, 404: 리소스가 없다(주소입력 잘못했음) NOT_FOUND
    ...
    /*
    - 어차피 RequestBody로 id 전달하면 되는거 아닌가?
    - 기술적으로는 RequestBody로 id 까지 객체로 데이터를 받아서 코딩 가능
    - RESTful 설계
        - URL: "어떤 자원을 다룰것인가?"(식별) -> id
        - METHOD: "뭘 할건가?"(행위) -> put(수정)
        - body: "어떤 데이터로?"(내용) -> 내가 보낸 dto로 바꿔줘
    */
    @PutMapping("/test4/{id}")
    ...
    // PUT -> 들어온 dto 데이터로 싹 다 바꾸겠다
    // PATCH -> 들어온 dto 데이터중 일부만 반영하겠다.

    // DELETE -> body 쓸 수 있는데, 보통 안씀
